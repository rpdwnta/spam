...
def banner():
 ...
 {k} *   * {h}/+/++//
   \ /  |/__//
 [{m}X{h}]{h}v[{m}X{h}]{p}|{b}RegiGiyaa{p}|==========.
   {h}({m}'{h})  {h}/'|'\           {p}\\
       {h}/  \  \          {p}'
       {h}\_  \_ \_   {p}___{m}Xtc Code Team {k}2.0{p}___
	
  
 {P}Author :{H}RegiGiyaa   Wa: 089509155154                                       ''')   

#--------------------[ BAGIAN-MASUK ]--------------#
def lisensi():
	lisensi = input("masukin lisensi nya: ")
	if lisensi ==( "MEMEK"):
		print("selamat akses di terima")
	else:
		print("lisensi salah")
		lisensi()
...
def menu(my_name,my_id):
	...
	io = '''[bold cyan][01] Crack Publik\n[02] Exit[bold cyan]'''
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='[bold cyan] • RegiGiyaa Crack • [/bold cyan]'))
...
def setting():
	io = '''[bold cyan][01] Tua 	 \n[02] Random''' 
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='[bold cyan] Seting [/bold cyan]'))
	hu = input(' pilih : ')
	if hu in ['1','01']:
	...
	io = '''[bold cyan][01] Metode B-Api.
[02] Metode Touch.\n[03] Metode Mbasic.
[04] Metode m.facebook.\n[05] Metode Iphone.
[06] Metode Web.facebook.\n[07] Metode facebook.
[08] Metode D.facebook.[bold cyan]'''
	oi = nel(io, style='cyan')
...
def passwrd():
	os.system('clear')
	jalan(f'{B}|└──> Xtc Termux Team {h}|└──>RegiGiyaa Wa :089509155154 ')
	print(f'|──> 𝐇𝐀𝐒𝐈𝐋 {h}𝐎𝐊{x} 𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 : {h}OK/%s {x}'%(okc))
	print(f'|──> 𝐇𝐀𝐒𝐈𝐋 {k}𝐂𝐏{x} 𝐓𝐄𝐑𝐒𝐈𝐌𝐏𝐀𝐍 𝐃𝐈 : {k}CP/%s {x}'%(cpc))
	print(f'|──> 𝐌𝐚𝐢𝐧𝐤𝐚𝐧 𝐌𝐨𝐝𝐞 𝐏𝐞𝐬𝐚𝐰𝐚𝐭 𝐒𝐞𝐭𝐢𝐚𝐩 {m}500{x} 𝐈𝐝𝐳\n')
